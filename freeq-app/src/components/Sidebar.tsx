import { getDeviceKeyState, subscribeDeviceKey } from '../irc/client';
...
export function Sidebar({ onOpenSettings }: SidebarProps) {
  const deviceKey = useSyncExternalStore(subscribeDeviceKey, getDeviceKeyState);
  ...
  // This device's key is not on the account yet; Settings is where that is
  // fixed, so the entry carries a dot.
  const keyUnpublished = !!authDid && !deviceKey.published;
      ...
      <div className="border-t border-border px-3 py-4 shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="text-fg-dim hover:text-fg-muted p-1 relative"
            title="Settings"
          >
            {/* This device's key is not published: the row that fixes it is
                inside Settings, so the entry carries the dot. */}
            {keyUnpublished && (
              <span
                data-testid="settings-key-dot"
                className="absolute top-0 right-0 w-1.5 h-1.5 rounded-full bg-accent"
              />
            )}
            <svg className="w-4 h-4" viewBox="0 0 16 16" fill="currentColor">
