...
Alongside the coordination events above, freeq is growing a refereed task family — `act-` tags on a TAGMSG, signed by the sender, checked against a rules file before the server accepts them. Two limits are worth knowing before you build on it, because neither is a bug to wait out.
