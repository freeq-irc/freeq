...
async fn main() -> anyhow::Result<()> {
    ...
    // Load bot identity
    let key_path = std::env::var("BOT_KEY").unwrap_or_else(|_| {
        let home = dirs::home_dir().unwrap();
        home.join(".freeq/bots/demo-bot/key.ed25519")
            .to_string_lossy()
            .to_string()
    });
    let identity_path = std::env::var("BOT_IDENTITY").unwrap_or_else(|_| {
        let home = dirs::home_dir().unwrap();
        home.join(".freeq/bots/demo-bot/identity.json")
            .to_string_lossy()
            .to_string()
    });
    ...
    let did = identity["did"]
        .as_str()
        .ok_or_else(|| anyhow::anyhow!("{identity_path}: no \"did\" string field"))?
        .to_string();
    ...
    let channel = std::env::var("IRC_CHANNEL").unwrap_or_else(|_| "#freeq".to_string());
    ...
    loop {
        match events.recv().await {
            Some(Event::Registered { nick }) => {
                ...
                // Set presence
                handle
                    .set_presence("active", Some("Ready to demo agent-native features"), None)
                    .await?;
                ...
                handle.privmsg(&channel, "👋 Hello from demo-bot! I'm an agent-native bot authenticated via did:key with SASL. Try /whois demo-bot to see my identity card.").await?;
            }
            Some(Event::Message {
                from, target, text, ..
            }) => {
                ...
                if addressed {
                    ...
                    match msg.to_lowercase().as_str() {
                        "ping" => {
                            handle
                                .privmsg(reply_target, &format!("{from}: pong! 🏓"))
                                .await?;
                        }
                        "status" => {
                        ...
                        "help" => {
                            handle
                                .privmsg(
                                    reply_target,
                                    &format!("{from}: Commands: ping, status, help, quit"),
                                )
                                .await?;
                        }
                        "quit" => {
                        ...
                        _ => {
                            handle
                                .privmsg(
                                    reply_target,
                                    &format!(
                                        "{from}: I don't understand that. Try: demo-bot: help"
                                    ),
                                )
                                .await?;
                        }
