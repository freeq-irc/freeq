...
- [x] **JOIN races PROVENANCE on connect** — ✅ DONE (instant-rooms branch). bot-kit no longer hands
  channels to the SDK autojoin: `FreeqBot` sends PROVENANCE first, waits (bounded, 3s) for the
  server's provenance NOTICE, then JOINs. Covers pi, freeqcc and `@freeq/mcp` (which now also
  actually sends PROVENANCE via `bot.start()`).
